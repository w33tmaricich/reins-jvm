(ns waldo.communications.spread
  (:require [waldo.utils.messages :as msg]
...
(defn connect
  ...
  (let [connection (SpreadConnection.)]
    (when (empty? connection-information)
        (msg/err 'connection-information "was empty. Please pass in a valid map")
        false)
    (when (not (valid-connection-information? connection-information connection-keys))
      (msg/err 'connection-information "does not have the proper fields. Please check keywords.")
      false)
    ...
    (try
      (do
        ...
        (msg/suc "Connection established")
        connection)
      (catch Exception e (msg/err (str "Connection failure: " (.getMessage e)))))))
...
(defn disconnect
  ...
  (try
    (do
      ...
      (msg/suc "Connection disconnected"))
    (catch Exception e (msg/err (str "Could not disconnect from spread: " (.getMessage e))))))
